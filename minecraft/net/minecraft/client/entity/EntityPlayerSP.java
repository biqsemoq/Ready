...
import xyz.semoteo.ready.cmds.Commands;
import xyz.semoteo.ready.cmds.CommandsManager;
import xyz.semoteo.ready.cmds.SettingCommands;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public final boolean isMoving() {
        return !(this.moveStrafing == 0.0F && this.moveForward == 0.0F);
      }
    ...
    {
    	   PlayerMotionEvent motionUpdate = new PlayerMotionEvent(this.rotationYaw, this.rotationPitch, this.posY, this.onGround, Event.State.BEFORE);
           motionUpdate.call();
        ...
        {
            double var9 = (double)(motionUpdate.getRotationYaw() - this.field_175164_bL);
            double var11 = (double)(motionUpdate.getRotationPitch() - this.field_175165_bM);
            boolean var13 = this.field_175168_bP >= 0;
            ...
            if (this.ridingEntity == null)
            {
                if (var13 && var14)
                {
                	C03PacketPlayer.C06PacketPlayerPosLook c06 = new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, motionUpdate.getY(), this.posZ, motionUpdate.getRotationYaw(), motionUpdate.getRotationPitch(), motionUpdate.ground);
                	motionUpdate.setPacket(c06);
                	sendUpdatePackets(c06,motionUpdate);
                }
                else if (var13)
                {
                    C03PacketPlayer.C04PacketPlayerPosition c04 = new C03PacketPlayer.C04PacketPlayerPosition(this.posX, motionUpdate.getY(), this.posZ, motionUpdate.ground);
                    motionUpdate.setPacket(c04);
                    sendUpdatePackets(c04,motionUpdate);
                }
                else if (var14)
                {
                    C03PacketPlayer.C05PacketPlayerLook c05 = new C03PacketPlayer.C05PacketPlayerLook(motionUpdate.getRotationYaw(), motionUpdate.getRotationPitch(), motionUpdate.ground);
                    motionUpdate.setPacket(c05);
                    sendUpdatePackets(c05,motionUpdate);
                }
                else
                {
                    C03PacketPlayer c03 = new C03PacketPlayer(motionUpdate.ground);
                    motionUpdate.setPacket(c03);
                    sendUpdatePackets(c03,motionUpdate);
                }
            }
            else
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, motionUpdate.getRotationYaw(), motionUpdate.getRotationPitch(), motionUpdate.ground));
                var13 = false;
            }
            ...
            if (var14)
            {
                this.field_175164_bL = motionUpdate.getRotationYaw();
                this.field_175165_bM = motionUpdate.getRotationPitch();
            }
            PlayerMotionEvent yoopostkibu = new PlayerMotionEvent(this.rotationYaw, this.rotationPitch, this.posY, this.onGround, Event.State.AFTER);
            yoopostkibu.call();
        }
        
    }
    
    private void sendUpdatePackets(Packet packet,PlayerMotionEvent event) {
    ...
    public void sendChatMessage(String message) {
        if (message.startsWith("-")) {
          ...
          try {
            Commands cmd = CommandsManager.getCommandFromText(message.contains(" ") ? message.split(" ")[0] : message);
            if (cmd == null) {
              boolean ananteo = false;
              for (Mod BAKMASANA_KODLARA_AMK : ModManager.getModules()) {
                if (BAKMASANA_KODLARA_AMK.name.equalsIgnoreCase(message.contains(" ") ? message.split(" ")[0] : message)) {
                  Ready.LOG((new SettingCommands()).RunCmd(message.contains(" ") ? message.split(" ") : null));
                  ananteo = true;
                  break;
                } 
              } 
              if (ananteo)
                return; 
              Ready.LOG("DUZGUN DUR ANANICIGINI SIKMIYIM KOMUT GIR");
            } 
            String receive = cmd.RunCmd(message.contains(" ") ? message.split(" ") : null);
            if (receive != null)
              Ready.LOG(receive); 
    ...
    public void onLivingUpdate()
    {
        ...
        SlowlessEvent slowlessEvent = new SlowlessEvent();
        slowlessEvent.call();
        if ((!slowlessEvent.isCancelled()) && (isUsingItem()) && (!isRiding()))
        {
