...
public class HttpPipelineConnection
{
    ...
    private static final Pattern patternFullUrl = Pattern.compile("^[a-zA-Z]+:// BAKMASANA KODLARA AMK
    ...
    private String normalizeUrl(String url, HttpRequest hr)
    {
        ...
        else if (url.startsWith("// BAKMASANA KODLARA AMK
        {
            return "http:" + url;
        }
        else
        {
            ...
            if (url.startsWith("/"))
            {
                return "http:// BAKMASANA KODLARA AMK
            }
            else
            {
                String file = hr.getFile();
                int pos = file.lastIndexOf("/");
                return pos >= 0 ? "http:// BAKMASANA KODLARA AMK
            }
        }
    }
