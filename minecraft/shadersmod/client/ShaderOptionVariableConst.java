...
public class ShaderOptionVariableConst extends ShaderOptionVariable
{
    ...
    private static final Pattern PATTERN_CONST = Pattern.compile("^\\s*const\\s*(float|int)\\s*([A-Za-z0-9_]+)\\s*=\\s*(-?[0-9\\.]+f?F?)\\s*;\\s*(// BAKMASANA KODLARA AMK
    ...
    {
        return "const " + this.type + " " + this.getName() + " = " + this.getValue() + "; // BAKMASANA KODLARA AMK
    }
